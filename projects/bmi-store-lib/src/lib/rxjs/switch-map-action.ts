...
import { httpCatchError } from 'bmi-api-lib';
...
export function switchMapAction8<S, T>(
  observableWithParams: (params: S) => Observable<T>,
  successCallback: (...arcs) => any,
  failureAction: Function
): OperatorFunction<S, Action> {
  return (source$: Observable<S>): Observable<Action> => source$.pipe(
    switchMap(params => observableWithParams(params).pipe(
      map((...resp) => successCallback(...resp)),
      httpCatchError((err) => failureAction({ err }))
    ))
  );
}
